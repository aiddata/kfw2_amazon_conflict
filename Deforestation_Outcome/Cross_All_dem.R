...
#dta_shp$prelevel_ntl <- dta_shp$ntl_1988
...
#This is the nighttime lights pretrend. Note that it only runs 1992-1988, not 1982-1988
#dta_shp$pretrend_ntl <- timeRangeTrend(dta_shp,"ntl_[0-9][0-9][0-9][0-9]",1992,1988,"id")
...
dta_shp$id_new <- rownames(dta_shp@data)
dta_shp$posttrend_ntl <- timeRangeTrend(dta_shp,"ntl_[0-9][0-9][0-9][0-9]",1992,2013, "id_new")
dta_shp$id_new <- NULL
...
dta_shp@data$NA_list[!is.na(dta_shp@data$demend_y) & (dta_shp@data$demend_y > 1987)] <- 0
dta_shp@data$Treat[dta_shp@data$NA_list == 0] <- 1


#Eliminate lands that were demarcated in years before 1988
...
aVars <- c("Treat", "terrai_are", "prelevel_pmean", "prelevel_pmin", "prelevel_pmax", "prelevel_tmean",
           ...
           "pretrend_pmean", "pretrend_pmin", "pretrend_pmax", "pretrend_tmean", "pretrend_tmin", "pretrend_tmax",
           "pretrend_ndvimean", "pretrend_ndvimax", "pretrend_pop", "Slope", "Elevation", "Riv_Dist", "Road_dist", "Pop_2000_y", "id") #"pretrend_ntl")
...
matchit.results <- matchit(Treat ~ terrai_are + prelevel_pmean + prelevel_pmin + prelevel_pmax + prelevel_tmean +
                             prelevel_tmin + prelevel_tmax + prelevel_ndvimean + prelevel_ndvimax + #prelevel_ntl + 
                             pretrend_pmean + pretrend_pmin + pretrend_pmax + pretrend_tmean + pretrend_tmin + pretrend_tmax + 
                             pretrend_ndvimean + pretrend_ndvimax + pretrend_pop + Slope + Elevation + Riv_Dist + Road_dist + Pop_2000_y, #+ pretrend_ntl 
                           data = dta_shp@data[aVars],
                           method = "nearest", distance="logit")
